...
public class PlayerControllerX : MonoBehaviour
{
    ...
    void Update()
    {
        //check if player is near upper boundary to prevent overshooting
    ...
    private void OnCollisionEnter(Collision other)
    {
        ...
        //bounce balloon off the ground on hit AND game is not over
        else if (other.gameObject.CompareTag("Ground") && !gameOver)
        {
            playerRb.AddForce(Vector3.up * 15, ForceMode.Impulse);
            playerAudio.PlayOneShot(groundBounce);
        }
